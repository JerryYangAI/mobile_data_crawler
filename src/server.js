...
import ExtractionAgent from './collector/ExtractionAgent.js';
import CollectionStore from './collector/CollectionStore.js';
import AutoCollector from './collector/AutoCollector.js';
import createCollectorRouter from './collector/routes.js';
...
// FSTea 手机端数据采集模式（只读截图 + 提取 + 落盘，独立于 Agent 动作链路）
const collectionStore = new CollectionStore(join(__dirname, '../data/collections'), orchestrator.logger);
const extractionAgent = new ExtractionAgent(orchestrator.doubaoAgent, orchestrator.logger);
const autoCollector = new AutoCollector({
  deviceManager: orchestrator.deviceManager,
  extractionAgent,
  store: collectionStore,
  model: config.doubao?.model,
  logger: orchestrator.logger
});
autoCollector.onEvent = (event) => io.emit('collector_event', event);
app.use('/api/collect', createCollectorRouter({
  deviceManager: orchestrator.deviceManager,
  extractionAgent,
  store: collectionStore,
  model: config.doubao?.model,
